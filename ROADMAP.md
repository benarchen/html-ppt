...
`v0.4.0` 是当前稳定版本及最新公开标签。Goal 004 图片型 PPTX 导出已完成并发布。Goal 005「跨平台命令与最小 CI」的范围内实现、Ubuntu／Windows CI、本地门禁和隔离安装均已完成，当前因缺少用户最终验收而处于 `blocked`。公开分发和有限可编辑 PPTX 继续延后。
...
- 当前没有正在施工的工程项；Goal 005 的工程范围已完成。
...
- Goal 005 连续三轮唯一剩余门禁均为用户明确验收，状态已更新为 `blocked`。恢复条件为用户回复「接受」或提供修改意见；此前不得合并 `main` 或创建 `v0.5.0` 标签。
