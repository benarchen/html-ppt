...
- Goal 005 当前为 `active`；本地跨平台入口、最小 CI、路径回归、文档候选和 `0.5.0` 最终门禁已完成，正在执行隔离安装，随后推送施工分支获取 Ubuntu／Windows 真实 CI 结果。

## 待办

- 完成 Goal 005 的隔离安装、Ubuntu／Windows 真实 CI 和用户验收。
...
- 2026-09-03：Goal 005 开发候选版本统一更新为 `0.5.0` 后重新执行最终本地门禁；类型检查、三主题契约、20 项浏览器回归和 4 组视觉回归全部通过，34 项非浏览器测试为 33 通过、1 项按设计跳过，且未更新视觉基线。隔离安装和远端 Ubuntu／Windows CI 待完成。
